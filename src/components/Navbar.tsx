...
import { Sun, Moon, Menu, X, UserCheck, Shield } from 'lucide-react';
...
export default function Navbar({
  ...
  const { isAuthorizedProf, logout } = useAuth();
  ...
  const navItems = [
    ...
    { label: 'Resources', view: isAuthorizedProf ? 'professor' : 'student' as const },
                ...
                {isAuthorizedProf ? (
                  <div className="flex items-center space-x-2">
                    <span className={`flex items-center space-x-1 border border-[#22201F]/20 ${badgeBg} px-2.5 py-1 text-xs font-medium text-[#22201F] dark:text-[#F6F2EA]/70 rounded-lg transition-colors duration-300`}>
                      <Shield size={14} className={`${accentText} transition-colors duration-300`} />
            ...
            <div>
              {isAuthorizedProf ? (
                <div className="flex items-center justify-between px-4 py-2 bg-[#F7F3EC] dark:bg-[#1A1817] rounded-xl border border-[#22201F]/20">
                  <span className="font-sans text-xs font-semibold text-[#22201F] dark:text-[#F6F2EA]/70 flex items-center gap-1.5">
