...
export const ResourceCard: React.FC<ResourceCardProps> = ({
  ...
  return (
    <div className="flex flex-col p-5 group transition-all duration-[220ms] hover:-translate-y-1 hover:shadow-[0_12px_24px_-12px_rgba(34,32,31,0.15)] rounded-[28px] border border-[#EAE1D2] bg-white shadow-[0_1px_2px_rgba(34,32,31,0.04),0_18px_36px_-26px_rgba(34,32,31,0.35)]">
      {image && (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <h4 className="text-sm font-bold text-[#22201F] line-clamp-2 group-hover:text-[#4A0E1B] transition-colors">
            {title}
          </h4>
        </div>
      </div>
          ...
          {actions.map((action, idx) => {
            ...
            if (action.variant === 'secondary') {
              return (
                <button 
                  key={idx} 
                  onClick={action.onClick}
                  className="inline-flex items-center justify-center gap-2 rounded-lg bg-[#F4E7E5] px-3 py-1.5 text-[11px] font-bold text-[#4A0E1B] transition-colors hover:bg-[#EEDAD7]"
                >
                  <ActionIcon size={12} /> {action.label}
                </button>
              );
            ...
            if (action.variant === 'ghost') {
              return (
                <button 
                  key={idx} 
                  onClick={action.onClick}
                  className="inline-flex items-center justify-center gap-2 rounded-lg border border-[#E3D8C5] bg-white px-3 py-1.5 text-[11px] font-semibold text-[#6E645A] transition-colors hover:bg-[#F6F2EA] hover:text-[#22201F]"
                >
                  <ActionIcon size={12} /> {action.label}
                </button>
              );
            ...
            return (
              <button 
                ...
                onClick={action.onClick} 
                title={action.label}
                className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#E3D8C5] bg-white text-[#6E645A] transition-colors hover:bg-[#F6F2EA] hover:text-[#22201F]"
              >
                <ActionIcon size={14} />
              </button>
            );
