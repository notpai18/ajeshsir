    ...
    return (
      <ModalShell onClose={onClose}>
        <div className="flex flex-1 flex-col items-center justify-center gap-4 p-8 text-center">
          ...
          <h4 className="dash-serif text-base font-semibold text-[#22201F]">Cannot load document</h4>
          <p className="max-w-xs text-sm text-[#8A7E6F]">{urlError}</p>
          <button
            onClick={onClose}
  ...
  return (
    ...
    >
      ...
      <button
        ...
        tabIndex={-1}
      />
      <div className="relative z-10 flex h-full flex-col overflow-hidden rounded-none bg-white sm:m-4 sm:rounded-2xl shadow-2xl">
        {children}
      </div>
    </div>
  );
  ...
  if (!resolvedUrl) {
    return (
      <>
        ...
        <div className="flex flex-1 items-center justify-center gap-3 bg-[#E8E0D5]">
          <div className="h-6 w-6 animate-spin rounded-full border-2 border-[#4A0E1B] border-t-transparent" />
          <p className="dash-root text-sm text-[#8A7E6F]">Resolving document…</p>
        </div>
      </>
    );
  }
