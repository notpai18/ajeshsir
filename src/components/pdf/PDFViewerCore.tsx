...
export function PDFViewerCore({ pdfUrl, containerWidth }: PDFViewerCoreProps) {
  ...
  // Theme backgrounds
  const viewerBg = theme === 'dark' ? 'bg-[#1a1a1a]' : theme === 'sepia' ? 'bg-[#f4ecd8]' : 'bg-[#F7F3EC] dark:bg-[#1A1817] dark:bg-[#1A1817] dark:bg-[#1A1817] dark:bg-[#1A1817]';
            ...
            Array.from({ length: numPages }, (_, i) => i + 1).map((pageNum) => (
              ...
              >
                <Page
                  ...
                  loading={
                    <div
                      className="animate-pulse rounded-sm bg-white dark:bg-[#22201F] dark:bg-[#22201F] dark:bg-[#22201F] dark:bg-[#22201F]"
                      style={{ width: pageWidth, height: Math.round(pageWidth * 1.414) }}
                    />
                  }
                  error={
                    <div className="flex items-center justify-center rounded-sm bg-white dark:bg-[#22201F] dark:bg-[#22201F] dark:bg-[#22201F] dark:bg-[#22201F] p-8 text-sm text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA] dark:text-[#F6F2EA] dark:text-[#F6F2EA]/60">
  ...
  return (
    <div className="flex flex-col items-center gap-4 py-8" aria-live="polite" aria-busy="true">
      {[1.414, 1.414, 1.0].map((ratio, i) => (
        <div
          key={i}
          className="w-full max-w-[600px] animate-pulse rounded-sm bg-white dark:bg-[#22201F] dark:bg-[#22201F] dark:bg-[#22201F] dark:bg-[#22201F]"
          style={{ height: Math.round(600 * ratio) }}
        >
          <div className="h-full w-full rounded-sm bg-gradient-to-b from-[#D9C2A2]/20 to-[#F7F3EC]/10" />
        </div>
      ))}
      <p className="dash-root text-xs text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA] dark:text-[#F6F2EA] dark:text-[#F6F2EA]/60 animate-pulse">Loading document…</p>
    </div>
  );
}
...
function ErrorState({ error, onRetry }: { error: string; onRetry: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center gap-4 py-20 text-center">
      ...
      <h4 className="dash-serif text-base font-bold text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA] dark:text-[#F6F2EA] dark:text-[#F6F2EA]">Could not load document</h4>
      <p className="max-w-xs text-sm text-[#22201F] dark:text-[#F6F2EA] dark:text-[#F6F2EA] dark:text-[#F6F2EA] dark:text-[#F6F2EA]/60">{error}</p>
      <button
        onClick={onRetry}
