...
export function FeaturedResource({ item, onPreview, onDownload }: { item: any, onPreview: () => void, onDownload: () => void }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-[32px] bg-white border border-[#EAE1D2] p-8 sm:p-10 shadow-[0_12px_40px_-12px_rgba(74,14,27,0.08)] group transition-all hover:shadow-[0_20px_50px_-12px_rgba(74,14,27,0.12)]">
      ...
      <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-8">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-4">
            <span className="inline-flex items-center gap-1.5 rounded-full bg-[#F7EFD9] px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-[#8A6A16]">
              🔥 Featured
            </span>
            <span className="inline-block rounded-full border border-[#EFE7D8] bg-[#FBF7F0] px-3 py-1 text-[10px] font-bold uppercase tracking-wider text-[#8A7E6F]">
              {item.chapter}
            </span>
            <SubjectBadge subject={item.subject} />
          </div>
          
          <h3 className="dash-serif text-2xl sm:text-3xl font-bold text-[#22201F] mb-3 group-hover:text-[#4A0E1B] transition-colors">
            {item.title}
          </h3>
          <p className="text-sm text-[#8A7E6F] leading-relaxed max-w-xl">
            {item.description}
          </p>
        </div>
        
        <div className="flex flex-row md:flex-col gap-3 shrink-0">
          <button onClick={onPreview} className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#F4E7E5] px-6 py-3.5 text-xs font-bold text-[#4A0E1B] transition-colors hover:bg-[#EEDAD7]">
            Preview
          </button>
...
export function ResourceOverview({ notesCount, videosCount, pyqsCount, sheetsCount }: any) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mt-8">
      ...
      <div className="relative rounded-[28px] bg-white border border-[#EAE1D2] p-6 flex flex-col items-center justify-center shadow-sm hover:-translate-y-1 transition-transform">
        ...
        <span className="text-[11px] font-bold uppercase tracking-widest text-[#8A7E6F]">Study Notes</span>
      </div>
      ...
      {/* Timeline for PYQs */}
      <div className="relative rounded-[28px] bg-white border border-[#EAE1D2] p-6 flex flex-col shadow-sm hover:-translate-y-1 transition-transform">
        <div className="flex items-center gap-3 mb-6">
          <div className="w-8 h-8 rounded-full bg-[#F7EFD9] text-[#8A6A16] flex items-center justify-center">
            <Clock size={16} />
          </div>
          <span className="text-[11px] font-bold uppercase tracking-widest text-[#8A7E6F]">Past Years</span>
        </div>
        <div className="flex-1 flex items-end">
          <div className="flex justify-between w-full items-end gap-1">
            {[40, 70, 45, 90, 60].map((h, i) => (
              <div key={i} className="w-full bg-[#F6F2EA] rounded-t-sm" style={{ height: '60px' }}>
      ...
      <div className="relative rounded-[28px] bg-white border border-[#EAE1D2] p-6 flex flex-col shadow-sm hover:-translate-y-1 transition-transform">
        <div className="flex items-center gap-3 mb-4">
          <div className="w-8 h-8 rounded-full bg-[#F4E7E5] text-[#4A0E1B] flex items-center justify-center">
            <FileText size={16} />
          </div>
          <span className="text-[11px] font-bold uppercase tracking-widest text-[#8A7E6F]">Drills</span>
        </div>
        <div className="mt-auto">
          <div className="flex justify-between items-end mb-2">
            <span className="text-3xl font-bold text-[#22201F] dash-mono">{sheetsCount}</span>
            <span className="text-[10px] text-[#8A7E6F] font-bold mb-1">Available</span>
          </div>
          <div className="w-full h-2 bg-[#F6F2EA] rounded-full overflow-hidden">
...
export function DownloadsLeaderboard({ items }: { items: any[] }) {
  ...
  return (
    <div className="bg-white rounded-[32px] border border-[#EAE1D2] p-8 shadow-sm">
      <div className="flex items-center gap-3 mb-8">
        <Trophy className="text-[#C9A13B]" size={24} />
        <h3 className="dash-serif text-2xl font-bold text-[#22201F]">Top Downloads</h3>
      </div>
          ...
          <div key={item.id} className="group flex items-center gap-4">
            ...
            <div className="flex-1">
              <div className="flex justify-between mb-1.5">
                <span className="text-sm font-semibold text-[#22201F] line-clamp-1 group-hover:text-[#4A0E1B] transition-colors">{item.title}</span>
                <span className="text-xs font-bold text-[#8A7E6F] dash-mono">{item.downloadCount}</span>
              </div>
              <div className="w-full h-1.5 bg-[#F6F2EA] rounded-full overflow-hidden">
                <div 
  ...
  return (
    <div className="bg-white rounded-[32px] border border-[#EAE1D2] p-8 shadow-sm">
      <div className="flex items-center gap-3 mb-8">
        ...
        <h3 className="dash-serif text-2xl font-bold text-[#22201F]">Latest Updates</h3>
      </div>
        ...
        {updates.slice(0, 4).map((update, idx) => (
          <div key={update.id} className="relative pl-6 group">
            ...
            <div className="flex items-center gap-2 mb-1">
              {update.pinned && <span className="text-[9px] font-bold uppercase tracking-widest text-[#4A0E1B] bg-[#F4E7E5] px-2 py-0.5 rounded-full">Pinned</span>}
            ...
            <h4 className="text-sm font-bold text-[#22201F] mb-1 group-hover:text-[#4A0E1B] transition-colors">{update.title}</h4>
            <p className="text-xs text-[#8A7E6F] leading-relaxed line-clamp-2">{update.body}</p>
          </div>
        ))}
      </div>
    </div>
