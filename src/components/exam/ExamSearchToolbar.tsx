    ...
    <div className="flex flex-col items-center gap-8 my-16 animate-[fadeInUp_0.8s_ease-out_forwards]">
      ...
      >
        ...
        <div className="relative flex items-center bg-white/70 backdrop-blur-2xl border border-white rounded-full p-2 shadow-[0_8px_30px_rgb(0,0,0,0.08)]">
          <div className={`flex items-center justify-center w-12 h-12 rounded-full transition-colors duration-500 ${isFocused ? 'bg-[#4A0E1B] text-white' : 'bg-transparent text-[#8A7E6F]'}`}>
          ...
          <input
            ...
            className="flex-1 bg-transparent border-none outline-none px-4 text-sm sm:text-base text-[#22201F] placeholder:text-[#A79A88]"
          />
        </div>
      </div>
        ...
        {subjects.map((subject) => (
          <button
            ...
            className={`relative z-10 px-6 py-2.5 rounded-full text-xs font-bold uppercase tracking-wider transition-colors duration-300 ${
              selectedSubject === subject ? 'text-white' : 'text-[#8A7E6F] hover:text-[#22201F]'
            }`}
          >
            {subject}
