...
### Brand — the five-color palette (the ONLY accents; no green, no blue, ever)
...
| **Wine — Crimson** | `#7C2532` | `text-[#7C2532]` `bg-[#7C2532]` | secondary accent: JEE-Advanced, Medium difficulty, "schedule" (soft bg `#F4E2E5`) |
| **Gold** (fill) | `#C9A13B` | `bg-[#C9A13B]` | dots, bar fills, solid gold marks (NEVER small text — too light, ~2:1) |
| **Gold** (text/icon) | `#8A6A16` | `text-[#8A6A16]` | gold text & icons: NEET, Easy, "resource", answered/success (soft bg `#F7EFD9`) |
| **Ink — Charcoal** | `#22201F` | `text-[#22201F]` | headings, key text, page text color, CSIR-NET identity |
| **Secondary — Warm Sand** | `#D9C2A2` | `bg-[#D9C2A2]` | hero accents (as `/10`–`/20` blobs), hero label text, M.Sc identity (`#C4A87F` dot) |
...
### Status & category colors (mapped onto the five palette colors — NO green, NO blue)
| Meaning | Text | Soft bg | Tailwind pair |
|---|---|---|---|
| Success / answered / Easy / "resource" (GOLD) | `#8A6A16` | `#F7EFD9` | `bg-[#F7EFD9] text-[#8A6A16]` |
| Warning / Medium / "schedule" (WINE) | `#7C2532` | `#F4E2E5` | `bg-[#F4E2E5] text-[#7C2532]` |
| Hard / "exam" (MAROON) | `#4A0E1B` | `#F4E4E4` | `bg-[#F4E4E4] text-[#4A0E1B]` |
...
> **No green, no blue.** Success/"done" reads as **gold**, not green; info/"schedule" reads as
> **wine**, not blue. The old palette used them — they are banned. Difficulty escalates gold → wine
> → maroon (Easy → Medium → Hard).
>
> **Danger ≠ Primary.** Maroon `#4A0E1B` is the brand; destructive actions use the one functional
> exception — brick red `#B23B2E` — so "delete" never reads as "primary." It is the *only* non-palette hue allowed.
...
const EXAM_STYLES: Record<string, { bg: string; text: string; dot: string }> = {
  ...
  'jee-advanced': { bg: 'bg-[#F4E2E5]', text: 'text-[#7C2532]', dot: 'bg-[#7C2532]' }, // wine
  neet:           { bg: 'bg-[#F7EFD9]', text: 'text-[#8A6A16]', dot: 'bg-[#C9A13B]' }, // gold
  net:            { bg: 'bg-[#ECE7E0]', text: 'text-[#22201F]', dot: 'bg-[#22201F]' }, // charcoal
  'msc-entrance': { bg: 'bg-[#EFE7D8]', text: 'text-[#6E5A2E]', dot: 'bg-[#C4A87F]' }, // sand
};

// announcement category chip
const ANN_CAT = {
  general:  { label: 'General',  cls: 'bg-[#EFE7D8] text-[#6E645A]' }, // neutral sand
  exam:     { label: 'Exam',     cls: 'bg-[#F4E4E4] text-[#4A0E1B]' }, // maroon
  resource: { label: 'Resource', cls: 'bg-[#F7EFD9] text-[#8A6A16]' }, // gold
  schedule: { label: 'Schedule', cls: 'bg-[#F4E2E5] text-[#7C2532]' }, // wine
};

// difficulty chip — escalates gold → wine → maroon
const DIFFICULTY = {
  Easy:   'bg-[#F7EFD9] text-[#8A6A16]', // gold
  Medium: 'bg-[#F4E2E5] text-[#7C2532]', // wine
  Hard:   'bg-[#F4E4E4] text-[#4A0E1B]', // maroon
};
```
...
<div className="flex flex-col items-center justify-center rounded-2xl border border-dashed border-[#E0D5C2] bg-[#FBF7F0] px-6 py-14 text-center">
  <div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-[#F7EFD9] text-[#8A6A16]"><FileText size={22} /></div>
  ...
  {open && (<>
    ...
    <div className="absolute right-0 z-50 mt-2 w-56 overflow-hidden rounded-2xl border border-[#EAE1D2] bg-white p-1.5 shadow-xl">
      <button className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-semibold text-[#4A443E] transition-colors hover:bg-[#F6F2EA]">
        <span className="text-[#8A6A16]"><FileText size={15} /></span> Study note
      </button>
    </div>
  </>)}
</div>
...
- Transitions are **`transition-colors`** on most interactive elements. Keep motion subtle; no bounce/spring.
- Chevrons rotate `rotate-180` when a menu/accordion opens.
- **Signature "slanted" card hover** — big *pickable* cards (exam cards, category cards) lift and tilt:
  add `transition-all duration-200 hover:-translate-y-1 hover:rotate-1` (alternate `hover:-rotate-1` on
  an adjacent group for playful variety). This tilt is reserved for whole-card link/button targets —
  do **not** apply it to content cards that contain their own buttons (notes, tables, etc.).
...
or `bg-[#F7EFD9] text-[#8A6A16]` (gold) for a secondary accent.
...
```
Reference implementation: `src/components/ProfessorDashboard.tsx`.
```
